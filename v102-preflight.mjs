...
need(server,'candidateSnapshotUpsert.run(JSON.stringify(payload),generatedAt,URL)','snapshot persistence');
const routeStart=server.indexOf("if (p === '/api/admin/candidates/refresh' && req.method === 'POST')"),routeEnd=server.indexOf("if (p === '/api/snapshot')",routeStart);if(routeStart<0||routeEnd<0)throw new Error('V1.0.2 preflight: candidate refresh route boundaries missing');const route=server.slice(routeStart,routeEnd);need(route,"user.role!=='admin'",'admin guard');need(route,'refreshCandidateCatalogFromTse()','TSE refresh call');for(const bad of ['insertRow.run(','updateRow.run(','deleteRow.run(','simInsertRow.run(','simUpdateRow.run('])if(route.includes(bad))throw new Error(`V1.0.2 preflight: candidate refresh touches election result storage via ${bad}`);
