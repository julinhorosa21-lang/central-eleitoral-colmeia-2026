...
.ce164-rank{position:absolute;right:7px;top:-8px;z-index:3;display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:21px;padding:0 7px;border:1px solid #CCD9DF;border-radius:999px;background:#F4F7F9;color:#49616F;font-size:10.5px;font-weight:900;box-shadow:0 2px 5px rgba(22,50,70,.05)}
...
function decorateRanking(){
  const rows=[...document.querySelectorAll('#leaders .leader')];
  rows.forEach(row=>{row.removeAttribute('data-rank');row.removeAttribute('data-ce161-rank');row.removeAttribute('data-ce164-rank');row.querySelectorAll('.ce161-rank,.ce159-rank,.ce154-rank,.ce164-rank').forEach(x=>x.remove());});
  ...
  scored.slice(0,3).forEach((x,i)=>{x.row.dataset.ce164Rank=String(i+1);const tag=document.createElement('span');tag.className='ce164-rank';tag.textContent=(i+1)+'º';tag.setAttribute('aria-label',(i+1)+'º colocado no momento');x.row.appendChild(tag);});
}
