...
function fix(){
 ...
 document.querySelectorAll('#leaders .leader').forEach(row=>{
   ...
   chip.classList.add('ce164-party-chip','ce167-party-chip');
   chip.setAttribute('data-party',text);chip.setAttribute('data-party-full',text);chip.setAttribute('title','Partido '+text);chip.setAttribute('aria-label','Partido '+text);chip.style.setProperty('--party',p);
...
write(`${pub}/v167-party-fix.js`,js);
for(const file of [`${pub}/index.html`,`${pub}/transparencia.html`]){
  let html=read(file);
  html=html.replace(/\s*<script src="\/v165-party-label-fix\.js[^\"]*"><\/script>/g,'');
  html=html.replace(/\s*<script src="\/v167-party-fix\.js[^\"]*"><\/script>/g,'');
  html=html.replace('</body>','<script src="/v167-party-fix.js?v=167"></script>\n</body>');
  write(file,html);
}
let sw=read(`${pub}/service-worker.js`);
sw=sw.replace(/const VERSION='[^']+';/,"const VERSION='v1.6.7-unified';");
if(!sw.includes("'/v167-party-fix.js'"))sw=sw.replace('const CORE=[',"const CORE=['/v167-party-fix.js',");
write(`${pub}/service-worker.js`,sw);
console.log('V1.6.7 applied: party labels now resolve from local CANDIDATOS first, with fetched catalog fallback.');
