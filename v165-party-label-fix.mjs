...
/* V1.6.8 — correção estrutural: selo partidário textual + PWA resiliente. */
...
function token(v){const s=norm(v);if(!s)return'';if(s.includes('UNIAO BRASIL')||/(^| )UNIAO( |$)/.test(s))return'UNIAO';for(const k of Object.keys(COLORS).sort((a,b)=>b.length-a.length))if((' '+s+' ').includes(' '+k+' '))return k;return''}
function label(v){const t=token(v);if(t==='UNIAO')return'UNIÃO';if(t==='MISSAO')return'MISSÃO';if(t==='PCDOB')return'PCdoB';return t||String(v||'').trim()||'PARTIDO'}
...
function candidate(cargo,n){const rows=catalog?.candidates?.[cargo];return Array.isArray(rows)?rows.find(c=>String(c?.numero||'').trim()===String(n).trim())||null:null}
function sourceFor(row,c){const legacy=row.querySelector('.who small .party-mark-v112');const aria=legacy?.getAttribute('aria-label')?.replace(/^Partido\\s+/i,'').trim();const title=legacy?.getAttribute('title')?.replace(/^Partido\\s+/i,'').trim();return c?.partido||c?.siglaPartido||c?.sgPartido||row.dataset.partyFull||aria||title||legacy?.dataset?.party||row.getAttribute('data-party')||''}
function ensureRow(row){
  ...
  const legacy=small.querySelector('.party-mark-v112');
  if(legacy){legacy.style.setProperty('display','none','important');legacy.setAttribute('aria-hidden','true')}
  let chip=small.querySelector('.ce168-party-text');
  if(!chip){chip=document.createElement('span');chip.className='ce168-party-text';small.insertBefore(chip,legacy||small.firstChild)}
...
for(const file of [`${pub}/index.html`,`${pub}/transparencia.html`]){
  let html=read(file);
  html=html.replace(/\/v165-party-label-fix\.js\?v=\d+/g,'/v165-party-label-fix.js?v=168');
  if(!html.includes('/v165-party-label-fix.js'))html=html.replace('</body>','<script src="/v165-party-label-fix.js?v=168"></script>\n</body>');
  write(file,html);
}
...
const sw=`const VERSION='v1.6.8-unified';
...
if(fs.existsSync(installPath)){
  ...
  s=s.replace(/async function ensurePwa\(\)\{[\s\S]*?\n\}/,`async function ensurePwa(){\n  if(!('serviceWorker' in navigator))return false;\n  try{\n    const reg=await navigator.serviceWorker.register('/service-worker.js?v=168',{scope:'/',updateViaCache:'none'});\n    try{await reg.update()}catch{}\n    await navigator.serviceWorker.ready;\n    return true;\n  }catch(err){console.error('PWA registration failed',err);return false}\n}`);
  s=s.replace("else setHelp('A Central está configurada como aplicativo. Se a janela não abrir automaticamente, atualize a página uma vez e toque novamente em “Instalar app”.');",`else if(/Firefox/i.test(ua))setHelp('Neste navegador a instalação automática pode não estar disponível. Abra o menu do navegador e procure “Instalar” ou “Adicionar à tela inicial”.');\n    else setHelp('O navegador ainda não liberou a janela automática. Abra o menu ⋮ e escolha “Instalar app” ou “Adicionar à tela inicial”.');`);
  write(installPath,s);
}

console.log('V1.6.8 applied: full-width party labels + resilient PWA installation.');
