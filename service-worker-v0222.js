...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keep=new Set([SHELL,RUNTIME,TILES]);for(const name of await caches.keys())if(!keep.has(name)&&name.startsWith('colmeia-'))await caches.delete(name);await self.clients.claim()})())});
...
async function networkFirst(req){const cache=await caches.open(RUNTIME);try{const r=await fetch(req);if(r&&r.ok)await cache.put(req,r.clone());return r}catch{const url=new URL(req.url);if(url.pathname==='/transparencia.html')return(await caches.match('/transparencia.html'))||(await caches.match('/index.html'))||Response.error();return(await cache.match(req))||(await caches.match('/index.html'))||Response.error()}}
async function cacheFirst(req,cacheName=RUNTIME){const cache=await caches.open(cacheName);const hit=await cache.match(req);if(hit)return hit;const r=await fetch(req);if(r&&(r.ok||r.type==='opaque')){await cache.put(req,r.clone());if(cacheName===TILES)trim(TILES,160)}return r}
