...
for(const f of [serverPath,packagePath,photoScriptPath,...pages]) if(!existsSync(f)) throw new Error(`V1.0.1 patch: missing ${f}`);

let server=readFileSync(serverPath,'utf8');
if(server.includes("version:'0.24.1'")) server=server.replace("version:'0.24.1'","version:'1.0.1'");
else if(!server.includes("version:'1.0.1'")) throw new Error('V1.0.1 patch: server version anchor missing');
...
if(!server.includes(stableLog)){
  if(!server.includes(photoLog)) throw new Error('V1.0.1 patch: startup log anchor missing');
...
}else if(!photoScript.includes(photoRunStable)) throw new Error('V1.0.1 patch: photo script version-update anchor missing');

for(const file of pages){
  let html=readFileSync(file,'utf8');
  if(!html.includes('/v100-main.js')){
    if(!html.includes('</body>')) throw new Error(`V1.0.1 patch: ${file} has no </body>`);
...
console.log('V1.0.1 stable hotfix patch applied.');
